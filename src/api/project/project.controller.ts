...
import { validateCreateProject, validateProjectQuery, validateUpdateProject } from "./project.validate";
...
export default class ProjectController {
 ...
 static getProjectById = async (req: express.Request, res: express.Response) => {
  try {
   const { id } = req.params;
  ...
  switch (true) {
   case !!req.query.pageId:
    try {
     ...
     const pageProjects = await ProjectServices.getProjectsByPageId(req.query.pageId as string);
   ...
   case !!req.query.membershipId:
    try {
     await MembershipService.getMembershipById(req.query.membershipId as string);

     const membershipProjects = await ProjectServices.getProjectsByMembershipId(req.query.membershipId as string);
 ...
 static updateProject = async (req: express.Request, res: express.Response) => {
  try {
   const { id } = req.params;
   const project: Project = req.body;
 ...
 static deleteProject = async (req: express.Request, res: express.Response) => {
  try {
   const { id } = req.params;
